...
data "template_file" "dockerfile" {
  template = file("./templates/Dockerfile.tpl")
  vars = {
    app_port = local.app_port
  }
}
resource "local_file" "dockerfile" {
  ...
  filename = "./repo-content/Dockerfile"
}
...
resource "null_resource" "upload_repo_code" {
  ...
  provisioner "local-exec" {
    command = "cd ./repo-content && rm -rf .git && git init && git add . && git commit -m 'first commit' && git branch -M master && git remote add origin https://github.com/${local.github_site}/${local.github_repo_name}.git && git push -u origin master"
    environment = {
